...
function initPublicFaqs() {
  ...
  const API_URL = '/ProjectPW-main/api/faqs/active';
  const limit = parseInt(list.getAttribute('data-faq-limit') || '0', 10);

  async function loadFaqs() {
    list.innerHTML = '<p>Carregando perguntas frequentes...</p>';
    ...
    try {
      ...
      if (!faqs.length) {
        list.innerHTML = '<p>Nenhuma pergunta frequente publicada no momento.</p>';
        return;
      }
      ...
      faqs.forEach((faq, index) => {
        ...
        faqItem.innerHTML = `
          <button class="accordion-trigger" type="button" aria-expanded="false" id="faq-${faq.id || index + 1}">
            <span></span><em class="accordion-icon" aria-hidden="true">▾</em>
          </button>
          <section class="accordion-content" role="region" aria-labelledby="faq-${faq.id || index + 1}"><p></p></section>`;

        faqItem.querySelector('span').textContent = faq.question;
        faqItem.querySelector('p').textContent = faq.answer;

        const button = faqItem.querySelector('.accordion-trigger');
      ...
      list.innerHTML = '<p>Nao foi possivel carregar as perguntas frequentes.</p>';
